...
// ── Project status mini-badge inside the summary popup ────────────────────
function ProjectStatusRow({ status }: { status: WorkerGlobalStatus['project_statuses'][0] }) {
  const dot =
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-slate-100 last:border-0">
      <span className="text-sm text-slate-700 truncate">{status.project_name}</span>
      <TrafficLightBadge status={status.traffic_light} showLabel />
    </div>
  )
}

// ── Popup: per-project breakdown ───────────────────────────────────────────
function WorkerSummaryPopup({
  worker,
  onClose,
}: {
  worker: WorkerGlobalStatus
  onClose: () => void
}) {
  return (
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="bg-slate-900 px-5 py-4 flex items-start justify-between">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-0.5">DNI: {worker.dni}</p>
          </div>
          <button
            onClick={onClose}
            className="mt-0.5 p-1.5 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white transition-colors"
          >
        ...
        <div className={`px-5 py-3 flex items-center gap-3 border-b ${
          worker.global_traffic_light === 'green' ? 'bg-green-50 border-green-100' :
          worker.global_traffic_light === 'yellow' ? 'bg-amber-50 border-amber-100' :
          worker.global_traffic_light ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-100'
        }`}>
          {worker.global_traffic_light ? (
            <>
              <TrafficLightBadge status={worker.global_traffic_light} size="md" showLabel />
              <span className="text-sm text-slate-600">Estado global</span>
            </>
          ) : (
            <span className="text-sm text-slate-500">Sin proyectos asignados</span>
          )}
        </div>
          ...
          {worker.project_statuses.length === 0 ? (
            ...
            <>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
                Estado por proyecto ({worker.project_statuses.length})
              </p>
              <div>
                {worker.project_statuses.map((ps) => (
                  <ProjectStatusRow key={ps.project_id} status={ps} />
                ))}
              </div>
            </>
          )}
        ...
        <div className="px-5 pb-5">
          <button
            onClick={onClose}
            className="w-full py-2.5 text-sm font-semibold rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors"
          >
            Cerrar
          </button>
        </div>
